...
export function LargeGBox({background, children}: {background: string, children: React.ReactNode}){
    ...
    return(
        <div
            ...
            <div
                className="h-100 z-10 absolute inset-0 w-full h-full bg-gradient-to-r from-gray-50 via-25% via-gray-50"></div>
            <div className="h-100 z-5 absolute right-0 top-0 bottom-0 h-full w-3/4 opacity-45"><img
                className="absolute h-full w-full object-cover select-none"
                draggable="false"
                src={background}/></div>
        </div>
